// All artwork is hand-built inline SVG — no external assets, no network fetch.
// Three distinct NPC portraits and an illustrated village backdrop.

export function Village() {
  return (
    <svg viewBox="0 0 1200 700" preserveAspectRatio="xMidYMid slice" className="village-svg" aria-hidden>
      <defs>
...
function Cottage({ x, y, c, roof, scale = 1 }) {
  return (
...
export function Portrait({ npc, size = 96, talking = false }) {
  const Inner = { aldric: AldricFace, maren: MarenFace, sable: SableFace }[npc] || AldricFace;
  return (
    <svg viewBox="0 0 120 120" width={size} height={size} className={`portrait ${talking ? "talking" : ""}`} aria-hidden>
      <Inner />
    </svg>
  );
}
