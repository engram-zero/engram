...
22. [Frontera expandible: el mapa crece casilla a casilla](#prompt-22--frontera-expandible-el-mapa-crece-casilla-a-casilla) — 🟡 milestone: frontier adjacency + walkable claimed cells + data-driven outer tiles + claim ghosts/labels
...
- Modelo puro de frontera en `world.ts`: `parcelIsClaimable`, `frontierClaimableCells`,
  `worldExtentForClaims`, `cellLabel`.
- El clamp de movimiento acepta `base playable ∪ claimed parcel cells`.
- Las parcelas reclamadas fuera del radio base renderizan su propio tile de suelo siguiendo
